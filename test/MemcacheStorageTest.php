...
use Vda\Multiprocess\Storage\MemcacheStorage;

class MemcacheStorageTestClass extends PHPUnit_Framework_TestCase
{
    public function testPutGetDelete()
    {
        $ss = new MemcacheStorage('localhost', 11211);
        $this->assertTrue($ss->put(getmypid(), 'test'));
        $this->assertEquals('test', $ss->get(getmypid()));
        $this->assertTrue($ss->remove(getmypid()));
    }

    public function testCrossProcessMemcacheStorage()
    {
        $pe = new \Vda\Multiprocess\ParallelExecutor(null, null, new MemcacheStorage('localhost', 11211));
